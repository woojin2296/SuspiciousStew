...
import { PatchnoteUiScene } from "./PatchNoteUi";
...
export class Stage4 extends Phaser.Scene {
  ...
  preload() {
    this.load.tilemapTiledJSON("map.level4", "/map/level4.json");
  }
  ...
  create() {
    ...
    const map = this.make.tilemap({ key: "map.level4" });
    ...
    this.patches = [];
    this.gameSetting = {
      SPAWN_POS: { col: 7, row: 9 },
      GOAL_POS: { col: 8, row: 2 },
      SLIME1_POS: { col: 13, row: 5 },
      SLIME2_POS: { col: 8, row: 5 },
      BOX_POS: { col: 8, row: 9 },
      KEY_POS: { col: 8, row: 8 },
      HAS_KEY: false,
      PLAYER_HP: 3,
    ...
    this.entitys = {
      ...
      slime1: { obj: new Slime(this, { col: this.gameSetting.SLIME1_POS.col, row: this.gameSetting.SLIME1_POS.row }), disabled: false, dir: "left", pos: { col: this.gameSetting.SLIME1_POS.col, row: this.gameSetting.SLIME1_POS.row } },
    ...
    // ⛔ 오타 수정: "gmae:input" -> "game:input"
    this.input.keyboard?.addCapture(["UP", "DOWN", "LEFT", "RIGHT"]);
    this.input.keyboard?.on("keydown-UP", () => {
      if (this.patches.includes(2)) flag = true;
      gameEventBus.emit("game:input", "up")
    });
    this.input.keyboard?.on("keydown-DOWN", () => {
      if (this.patches.includes(2)) flag = true;
      gameEventBus.emit("game:input", "down")
    });
    this.input.keyboard?.on("keydown-LEFT", () => {
      if (this.patches.includes(2)) flag = true;
      gameEventBus.emit("game:input", "left")
    });
    this.input.keyboard?.on("keydown-RIGHT", () => {
      if (this.patches.includes(2)) flag = true;
      gameEventBus.emit("game:input", "right")
    });
    ...
    gameEventBus.on("game:input", (key: "left" | "right" | "up" | "down") => {
      ...
      let [nsx1, nsy1] = [this.entitys.slime1.pos.col + dsc1, this.entitys.slime1.pos.row + dsr1];

      const [dsc2, dsr2] = { left: [-1, 0], right: [1, 0], up: [0, -1], down: [0, 1] }[this.entitys.slime2.dir as "left" | "right" | "up" | "down"];
      let [nsx2, nsy2] = [this.entitys.slime2.pos.col + dsc2, this.entitys.slime2.pos.row + dsr2];

      if (!this.patches.includes(1)) {
        if (isWall(nsx1, nsy1)) {
          nsx1 = this.entitys.slime1.pos.col;
          nsy1 = this.entitys.slime1.pos.row;
        }
        if (isWall(nsx2, nsy2)) {
          nsx2 = this.entitys.slime2.pos.col;
          nsy2 = this.entitys.slime2.pos.row;
        }
      }
      ...
      if (!this.entitys.slime1.disabled) {
        ...
        if (!this.patches.includes(1) && isWall(nsx, nsy)) {
    ...
    gameEventBus.on("game:proceed", async (nx: number, ny: number, tags: string[], key: string) => {
      ...
      if (this.patches.includes(2) && flag) {
