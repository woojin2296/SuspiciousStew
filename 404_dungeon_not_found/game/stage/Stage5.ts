...
export class Stage5 extends Phaser.Scene {
  ...
  preload() {
    this.load.tilemapTiledJSON("map.level5", "/map/level5.json");
  }
  ...
  create() {
    ...
    const map = this.make.tilemap({ key: "map.level5" });
    ...
    for (const { name, depth } of layerOrder) {
      const layer = map.createLayer(name, ts, 0, 16);
      if (layer) {
    ...
    this.input.keyboard?.on("keydown-LEFT", () => {
      flag = true;
      gameEventBus.emit("gmae:input", "left")
    });
    this.input.keyboard?.on("keydown-RIGHT", () => {
      flag = true;
      gameEventBus.emit("gmae:input", "right")
    });
    this.input.keyboard?.on("keydown-UP", () => {

      flag = true;
      gameEventBus.emit("gmae:input", "up")
    });
    this.input.keyboard?.on("keydown-DOWN", () => {
      flag = true;
      gameEventBus.emit("gmae:input", "down")
    });
    ...
    gameEventBus.on("game:proceed", async (nx: number, ny: number, tags: string[], key: string) => {
      ...
      if (this.patches.includes(2) && flag) {
        gameEventBus.emit("gmae:input", key)
        flag = false
      }
    });
