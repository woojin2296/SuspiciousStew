...
export class Stage2 extends Phaser.Scene {
  ...
  create() {
    ...
    this.gameSetting = {
      SPAWN_POS: { col: 10, row: 9 },
      GOAL_POS: { col: 10, row: 2 },
      KEY_POS: { col: 10, row: 6 },
      SLIME_POS: { col: 7, row: 6 },
      HAS_KEY: false,
      PLAYER_HP: 3,
      MOVE_LIMIT: 10,
    }
    this.entitys = {
      ...
      slime: { obj: new Slime(this, { col: this.gameSetting.SLIME_POS.col, row: this.gameSetting.SLIME_POS.row }), disabled: false, dir: "right", pos: { col: this.gameSetting.SLIME_POS.col, row: this.gameSetting.SLIME_POS.row } },
    }
    this.isMoving = false;
    ...
    gameEventBus.on("gmae:input", (key: "left" | "right" | "up" | "down") => {
      ...
      const { dsc, dsr } = { left: { dsc: -1, dsr: 0 }, right: { dsc: 1, dsr: 0 }, up: { dsc: 0, dsr: -1 }, down: { dsc: 0, dsr: 1 } }[this.entitys.slime.dir as "left" | "right" | "up" | "down"];
      const [nsx, nsy] = [this.entitys.slime.pos.col + dsc, this.entitys.slime.pos.row + dsr];
      ...
      const isSlime = (col: number, row: number) => { return col === nsx && row === nsy && !this.entitys.slime.disabled; }
      ...
      if (!this.entitys.slime.disabled) {
        let { nsx, nsy } = { left: { nsx: this.entitys.slime.pos.col - 1, nsy: this.entitys.slime.pos.row }, right: { nsx: this.entitys.slime.pos.col + 1, nsy: this.entitys.slime.pos.row }, up: { nsx: this.entitys.slime.pos.col, nsy: this.entitys.slime.pos.row - 1 }, down: { nsx: this.entitys.slime.pos.col, nsy: this.entitys.slime.pos.row + 1 } }[this.entitys.slime.dir as "left" | "right" | "up" | "down"];
        console.log(nsx, nsy, isWall(nsx, nsy));
        if (isWall(nsx, nsy)) {
          this.entitys.slime.dir = this.entitys.slime.dir === "left" ? "right" : this.entitys.slime.dir === "right" ? "left" : this.entitys.slime.dir === "up" ? "down" : "up";
          nsx = this.entitys.slime.pos.col + (this.entitys.slime.dir === "left" ? -1 : this.entitys.slime.dir === "right" ? 1 : 0);
          nsy = this.entitys.slime.pos.row + (this.entitys.slime.dir === "up" ? -1 : this.entitys.slime.dir === "down" ? 1 : 0);
        }
        this.entitys.slime.obj?.moveTo({ col: nsx, row: nsy }, this.entitys.slime.dir as "left" | "right" | "up" | "down");
        this.entitys.slime.pos = { col: nsx, row: nsy };
      }
